...
export class AuthService extends HttpService {
    ...
    constructor() {
        ...
        if (!env.useMockData) {
            ...
            if (token === 'mock-jwt-token') {
                authLogger.warn('Clearing mock token in API mode');
    ...
    async login(email: string, password: string): Promise<LoginResponse> {
        if (env.useMockData) {
            authLogger.info('Using Mock Login');
        ...
        try {
            const response = await this.post<BackendLoginResponse>(API_ENDPOINTS.AUTH.LOGIN, {
                email,
                password
            });
    ...
    async register(data: RegisterRequest): Promise<RegisterResponse> {
        if (env.useMockData) {
            authLogger.info('Using Mock Register');
    ...
    async logout(): Promise<void> {
        ...
        try {
            ...
            authLogger.warn('Server logout failed, clearing local session', err);
        }
    ...
    async setOAuthToken(token: string): Promise<void> {
        ...
        try {
            ...
            authLogger.error('Failed to load user profile after OAuth', error);
